...
local function TweenTo(targetPos, speed)
	...
	navService.targetPos = targetPos
	navService.hum = hum
...
local function NavWalk(hum, pos)
	...
	navService.isActive = true
	if togs.farm.moveMethod == "Tween" then
		TweenTo(pos, togs.farm.tweenSpeed)
	else
		hum:MoveTo(pos)
	end
	return true
...
local nodes = {
	...
	polar_bear = Vector3.new(-106, 119, -77), _polar_bear = Vector3.new(-80.68696594238281, 117.99800109863281, -78.48657989501953),
	start_area_to_area_15_jump = Vector3.new(-255, 4, 210),
	...
	_25_bee_gate = Vector3.new(98.7481460571289, 175.99803161621094, -122.87193298339844),
...
local nodeLinks = {
	...
	area_25_ramp_final = {"area_25_ramp_3", "_25_bee_gate"},
	_25_bee_gate = {"area_25_ramp_final"},
	polar_bear = {"area_25_ramp_3", "_polar_bear", "beesmas_feast_jump"},
	_polar_bear = {"polar_bear"},
...
local function FollowPath(pathList, field)
	...
	currentPathField = field
	currentPathGoal = field and fieldHubs[field] or "spawn"
	StopNav()
	task.wait(0.3)
	local char = LocalPlayer.Character
	if not char then isFollowingPath = false; currentPath = nil; return false end
	local hum = char:FindFirstChild("Humanoid")
	if not hum then isFollowingPath = false; currentPath = nil; return false end
	StartJumpThread()
	for _, node in ipairs(pathList) do
		local tpos = nodes[node]
		if not tpos then StopJumpThread(); isFollowingPath = false; currentPath = nil; return false end
		navService.isActive = true
		navService.targetPos = tpos
		navService.hum = hum
		navService.char = char
		if togs.farm.moveMethod == "Tween" then
			TweenTo(tpos, togs.farm.tweenSpeed)
		else
			hum:MoveTo(tpos)
			local st = os.clock()
		...
		navService.isActive = false
		if node == "redcannonfinal" and field and cannonFields[field] then
			SendKey(Enum.KeyCode.E)
			task.wait(0.5)
			if field == "Mountain Top Field" then
				StopJumpThread(); activeField = nil; isFollowingPath = false; currentPath = nil; return true
			else
				SendKey(Enum.KeyCode.Space)
				task.wait(0.05)
				SendKey(Enum.KeyCode.Space)
	...
	StopJumpThread(); activeField = nil; isFollowingPath = false; currentPath = nil
	return true
end

local function RestartCurrentPath()
	if not currentPath or not currentPathGoal then return end
	local char = LocalPlayer.Character
	if not char then return end
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if not hrp then return end
	...
	if bestNode then
		local route = FindPath(bestNode, currentPathGoal)
		if route then
			FollowPath(route, currentPathField)
			char.Humanoid:MoveTo(Workspace.FlowerZones:FindFirstChild(togs.farm.field).Position)
		end
	end
end
...
local function GotoField(fName)
	...
	if not hrp then return end
	if togs.farm.moveMethod == "Tween" then
		local field = Workspace.FlowerZones:FindFirstChild(fName)
		if field then
			TweenTo(field.Position + Vector3.new(0, 5, 0), togs.farm.tweenSpeed)
		end
		return
	end
	local bestNode, bestDist = nil, math.huge
...
local function GotoSpawn()
	...
	if not hrp then return false end
	if togs.farm.moveMethod == "Tween" then
		TweenTo(spawnPos.Position, togs.farm.tweenSpeed)
		return true
	end
	local bestNode, bestDist = nil, math.huge
	for nName, nPos in pairs(nodes) do
...
LeftGroupFarm:AddDropdown("MoveMethodDropdown", {
	Text = "Move Method",
	Values = {"Walk", "Tween"},
	Default = "Walk",
	Multi = false,
	Tooltip = "Movement method for navigation.",
	Callback = function(Value)
		togs.farm.moveMethod = Value
	end
})

LeftGroupFarm:AddSlider("TweenSpeedSlider", {
	Text = "Tween Speed",
	Default = 6,
	Min = 1,
	Max = 12,
	Rounding = 1,
	Tooltip = "Speed multiplier for Tween movement.",
	Callback = function(Value)
		togs.farm.tweenSpeed = Value
	end
})
...
LeftGroupFarm:AddToggle("FarmField", {
	...
	Callback = function(Value)
		...
		LocalPlayer:WaitForChild("PlayerScripts").ControlScript.Disabled = Value
		if not Value then
			StopPatternThread()
			StopTokenThread()
			sprinklersPlaced = false
